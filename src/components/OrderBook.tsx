    ...
    getOrderBook(propertyId)
      .then((data) => {
        ...
        const bids = (data.bids || []).map((b: any) => ({
          price: b.price,
          quantity: b.total_amount ?? b.amount ?? 0,
          total: (b.price || 0) * (b.total_amount ?? b.amount ?? 0),
        }));
        const asks = (data.asks || []).map((a: any) => ({
          price: a.price,
          quantity: a.total_amount ?? a.amount ?? 0,
          total: (a.price || 0) * (a.total_amount ?? a.amount ?? 0),
        }));
